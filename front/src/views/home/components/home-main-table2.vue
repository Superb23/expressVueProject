<template>
  <div class="main">
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="cardNumber" label="电话号码" width="180" />
    ...
    >
      <el-form :model="currentEdict" ref="ruleFormRef">
        ...
        <el-form-item label="电话号码" prop="cardNumber" required>
          <el-input v-model="currentEdict.cardNumber"></el-input>
        </el-form-item>
...
const currentChange = (btn = "") => {
  if (btn === "pre") {
    ...
    if (clientData.value.page < Math.ceil(clientData.value.total / 16)) {
      clientData.value.page++;
...
// 启用/禁用
const StartOrStophandle = async(index, row) => {
  const status = row.status === 1 ? 0 : 1;
  const code = await fetchClientUpdateStatus({
    id: row.id,
    status,
  });
  if (code === 1) {
...
const fetchClientUpdateStatus = async(query) => {
  const res = (await getClientUpdateStatus(query)).data;
  return res?.code;
}

const fetchClientUpdate = async(query) => {
  const res = (await getClientUpdate(query)).data;
  return res?.code;
}
...
const confirmClick = async(value) => {
  ...
  const code = await fetchClientUpdate({
    id: value.id,
    name: value.name,
    sex: value.sex,
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      formData.value = {
        ...
        cardNumber: currentEdict.value.cardNumber,
...
const fetchClientDelete = async(query) => {
  const res = (await getClientDelete(query)).data;
  return res?.code;
}
const deleteHandle = async(row) => {
  ...
  const code = await fetchClientDelete({
    id: row.id
  });
  if (code === 1) {
    ElMessage({
